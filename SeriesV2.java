public class SeriesV2<T> implements Series<T> {
    ...
    public String toString() {
        String llOutput = seriesData.toString();
        return llOutput.replace("print the linked list ...", "print the series ...");
    }
    ...
    public T iloc(int ind) {
        try {
            String[] indices = seriesData.getIndexArray();
            if (ind < 0 || ind >= indices.length) {
                throw new IndexOutOfBoundsException();
            }
            return loc(indices[ind]);
    ...
    public void fillNull(T value)
        ...
        for (String index : indices) {
            T currentData = loc(index);
            if (currentData == null) {
                seriesData.updateNode(index, value);
                seriesDataBST.updateNode(index, value);
            }
        }
    }
}